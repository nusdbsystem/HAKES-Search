...
#include <faiss/ext/TagChecker.h>
...
struct ReservoirHandler : ResultHandlerCompare<C, with_id_map> {
    ...
    inline std::string to_string(bool all_data = false) const override {
        std::stringstream s;
        s << "ReservoirHandler: nq=" << this->nq << " capacity=" << capacity;
        for (auto& res: reservoirs) {
            s << "\n  " << res.to_string(all_data);
        }
        return s.str();
    }
};
