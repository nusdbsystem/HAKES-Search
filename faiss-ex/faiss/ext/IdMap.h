...
#ifndef HAKES_IDMAP_H_
#define HAKES_IDMAP_H_
...
class IDMap {
 public:
  ...
  virtual void reserve(faiss::idx_t n) = 0;

  inline virtual bool load(const char* path, int io_flags = 0) {
    FileIOReader reader(path);
    return load(&reader, io_flags);
  };
  virtual bool load(IOReader* f, int io_flags = 0) = 0;

  inline virtual bool save(const char* path) const {
    FileIOWriter writer(path);
    return save(&writer);
  }
...
class IDMapImpl : public IDMap {
 public:
  ...
  void reserve(faiss::idx_t n) override;
