...
void IndexIVFFastScanL::init_fastscan(size_t M, size_t nbits, size_t nlist,
  ...
  FAISS_THROW_IF_NOT(nbits == 4);  // nbits now must be 4
...
void IndexIVFFastScanL::search_dispatch_implem(
  ...
  if (impl == 1) {
    ...
    if (is_max) {
      search_implem_2<CMax<uint16_t, int64_t>>(n, x, k, distances, labels, cq,
                                               scaler);
    } else {
      search_implem_2<CMin<uint16_t, int64_t>>(n, x, k, distances, labels, cq,
                                               scaler);
    }
...
#pragma omp parallel for reduction(+ : ndis, nlist_visited)
  for (idx_t i = 0; i < n; i++) {
    ...
    for (idx_t j = 0; j < nprobe; j++) {
      ...
      estimators_from_tables_generic<C>(*this, codes.get(), ls, LUT, ids.get(),
