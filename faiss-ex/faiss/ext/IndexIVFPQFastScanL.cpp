...
IndexIVFPQFastScanL::IndexIVFPQFastScanL(
        Index* quantizer,
        size_t d,
        size_t nlist,
        size_t M,
...
IndexIVFPQFastScanL::IndexIVFPQFastScanL(
        ...
        size_t M,
        size_t nbits,
        bool balanced_assign,
        int balance_k,
        MetricType metric,
        int bbs)
        : IndexIVFFastScanL(quantizer, d, nlist, 0, balanced_assign, balance_k, metric), pq(d, M, nbits) {
    by_residual = false; // set to false by default because it's faster
...
void IndexIVFPQFastScanL::train_encoder(
        idx_t n,
        const float* x,
        const idx_t* assign) {
    pq.verbose = verbose;
...
void IndexIVFPQFastScanL::encode_vectors(
        idx_t n,
        const float* x,
...
void IndexIVFPQFastScanL::compute_LUT(
        size_t n,
        const float* x,
...
#pragma omp parallel for if (n * nprobe > 8000)
                ...
                qpq_ref.compute_distance_tables(
...
void IndexIVFPQFastScanL::sa_decode(idx_t n, const uint8_t* bytes, float* x)
        const {
    pq.decode(bytes, x, n);
}
