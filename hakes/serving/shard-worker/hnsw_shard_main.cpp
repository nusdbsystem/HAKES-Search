...
int main(int argc, char* argv[]) {
  ...
  if (argc != 6) {
    fprintf(stderr, "Usage: port data_path d capacity ef_search\n");
    exit(1);
  }
  // parse the port
  ...
  uint64_t capacity = std::stoull(argv[4]);
  int ef_construction = std::stoi(argv[5]);

  hakes::ShardEngine* engine =
      new hakes::HnswShardEngine(d, capacity, ef_construction);
