...
bool HakesBatchFnREngine::Initialize(const std::string& index_path,
  ...
  search_task_queue_.reset(new HakesSearchTaskQueue(executor_.get()));
  return true;
}

bool HakesBatchFnREngine::IsInitialized() { return executor_->IsInitialized(); }

bool HakesBatchFnREngine::Add(const std::string& request,
                              std::string* response) {
...
bool HakesBatchFnREngine::Search(const std::string& request,
                                 std::string* response) {
  auto start_time = std::chrono::high_resolution_clock::now();
  assert(response);
  ...
  hakes::HakesSearchTask search_task(search_request, &search_response);
  search_task_queue_->Schedule(&search_task);
  success = search_response.status;

  encode_search_response(std::move(search_response), response);
...
bool HakesBatchFnREngine::Rerank(const std::string& request,
                                 std::string* response) {
  auto start_time = std::chrono::high_resolution_clock::now();
  assert(response);
  SearchResponse search_response;
...
bool HakesBatchFnREngine::GetIndex(std::string* response) {
...
bool HakesBatchFnREngine::UpdateIndex(const std::string& request,
                                      std::string* response) {
  auto start_time = std::chrono::high_resolution_clock::now();
  assert(response);
...
bool HakesBatchFnREngine::UpdateIndexLocal(const std::string& local_path) {
