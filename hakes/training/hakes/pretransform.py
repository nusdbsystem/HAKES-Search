...
from hakes.convert import matrix_to_str
...
class HakesPreTransform(torch.nn.Module):
    ...
    def save(self, save_path, ivf_vt=False):
        save_path = (
            f"{save_path}/pre-transform"
            if not ivf_vt
            else f"{save_path}/ivf_pre-transform"
        )
        logging.info(f"Saving PreTransform to {save_path}")
        os.makedirs(save_path, exist_ok=True)
        for i, vt in enumerate(self.vt_list):
            np.save(
                os.path.join(save_path, f"vt_{i}_A.npy"), vt.A.detach().cpu().numpy()
            )
            np.save(
                os.path.join(save_path, f"vt_{i}_b.npy"), vt.b.detach().cpu().numpy()
            )
